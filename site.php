...
$app->get("/categories/:idcategory", function($idcategory){
	...
	$pagination = $category->getProductsPage($page, 10);
...
$app->post('/register', function(){
	...
	if(User::checkLoginExist($_POST['email'])){
		User::setErrorRegister("Usuario existente");
		header("Location: /login");
		exit;
	}
...
$app->post("/profile", function(){
	...
	if(!isset($_POST['desperson']) || $_POST['desperson'] === ''){
		User::setError("Preenncha o seu nome.");
		header("Location: /profile");
		exit;
	}
	if(!isset($_POST['desemail']) || $_POST['desemail'] === ''){
		User::setError("Preenncha o seu email.");
		header("Location: /profile");
		exit;
	}
	...
	if($_POST['desemail'] !== $user->getdesemail()){
		if(User::checkLoginExist($_POST['desemail'])){
			User::setError("Este email ja esta cadastrado");
			header("Location: /profile");
			exit;
		}
	}
	...
	User::setSuccess("Auteracao feita com sucesso!");
...
$app->get("/boleto/:idorder", function($idorder){
	...
	$dadosboleto["instrucoes3"] = "- Em caso de dúvidas entre em contato conosco: suporte@suporte.com.br";
	...
	$dadosboleto["endereco"] = "Rua Ademar Saraiva Leão, 000 - Alvarenga, 09853-120";
...
$app->post("/profile/change-password", function(){
	...
	if($_POST['current_pass'] === $_POST['new_pass']){
		User::setError("Nova senha deve ser diferente da atual");
		header("Location: /profile/change-password");
		exit;
	}
